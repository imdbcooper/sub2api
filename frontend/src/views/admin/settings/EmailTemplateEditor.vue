...
const eventDisplayMetaKeys: Record<string, { key: string; category: string }> = {
  "auth.verify_code": { key: "authVerifyCode", category: "auth" },
  "auth.password_reset": { key: "authPasswordReset", category: "auth" },
  "notification_email.verify_code": { key: "notificationEmailVerifyCode", category: "auth" },
  "subscription.purchase_success": { key: "subscriptionPurchaseSuccess", category: "subscription" },
  "subscription.expiry_reminder": { key: "subscriptionExpiryReminder", category: "subscription" },
  "balance.low": { key: "balanceLow", category: "billing" },
  "balance.recharge_success": { key: "balanceRechargeSuccess", category: "billing" },
  "account.quota_alert": { key: "accountQuotaAlert", category: "admin" },
  "content_moderation.violation_notice": { key: "contentModerationViolationNotice", category: "risk_control" },
  "content_moderation.account_disabled": { key: "contentModerationAccountDisabled", category: "risk_control" },
  "ops.alert": { key: "opsAlert", category: "ops" },
  "ops.scheduled_report": { key: "opsScheduledReport", category: "ops" },
};
...
function eventMetaFor(option?: EmailTemplateOption | null) {
  ...
  const displayMeta = eventDisplayMetaKeys[option.value];
  const label = displayMeta
    ? t(`admin.settings.emailTemplates.events.${displayMeta.key}.label`)
    : option.label || option.value;
  const timing = displayMeta
    ? t(`admin.settings.emailTemplates.events.${displayMeta.key}.timing`)
    : option.description || "";
  const categoryLabel =
    (displayMeta ? formatCategory(displayMeta.category) : null) ||
