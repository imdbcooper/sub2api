      ...
      >
        ...
        >
          ...
          <div class="flex items-center justify-between">
            <div>
              <label class="text-sm text-gray-600 dark:text-gray-400"
                >{{ t("admin.groups.accountFilter.oauthOnly.label") }}</label
              >
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{
                  createForm.require_oauth_only
                    ? t("admin.groups.accountFilter.oauthOnly.enabledHint")
                    : t("admin.groups.accountFilter.notEnabled")
                }}
              </p>
            </div>
            <button
          ...
          <div class="flex items-center justify-between">
            <div>
              <label class="text-sm text-gray-600 dark:text-gray-400"
                >{{ t("admin.groups.accountFilter.privacySet.label") }}</label
              >
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{
                  createForm.require_privacy_set
                    ? t("admin.groups.accountFilter.privacySet.enabledHint")
                    : t("admin.groups.accountFilter.notEnabled")
                }}
              </p>
            </div>
...
const copyAccountsGroupOptions = computed(() => {
  ...
  return eligibleGroups.map((g) => ({
    value: g.id,
    label: t("admin.groups.copyAccounts.optionLabel", {
      name: g.name,
      count: g.account_count || 0,
    }),
  }));
});
