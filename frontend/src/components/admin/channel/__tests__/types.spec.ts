...
describe('validateIntervals', () => {
  describe('token mode', () => {
    it('rejects unbounded interval that is not last', () => {
      ...
      expect(validateIntervals(intervals, 'token')).toMatch(/unlimited interval/)
    })
    ...
    it('defaults mode to token when omitted', () => {
      ...
      expect(validateIntervals(intervals)).toMatch(/unlimited interval/)
    })
  })
  ...
  describe('image / per_request mode', () => {
    ...
    it('still rejects max <= min on a single tier', () => {
      ...
      expect(validateIntervals(intervals, 'image')).toMatch(/must be greater than/)
    })
  })
})
